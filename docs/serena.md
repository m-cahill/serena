...
|-----------|-------|--------|
| M34 | Runtime context model-identity seam | In progress |
...
**Progress (Phase VIII):** **M34** **in progress** on **`m34-runtime-context-model-identity`** — explicit **`ModelIdentity`** on **`RuntimeContext`** (`docs/milestones/M34/`). **M35–M37** **planned**. Authoritative scope: **`docs/serenav1audit.md`**.
...
| M34 | Runtime context model-identity seam | In progress | m34-runtime-context-model-identity | **[#90](https://github.com/m-cahill/serena/pull/90)** | — | — | **`ModelIdentity`** on **`RuntimeContext`**; see **`docs/milestones/M34/M34_plan.md`** | — |
...
**M34:** **Runtime context model-identity seam** — branch **`m34-runtime-context-model-identity`**; **`ModelIdentity`** (`name_for_extra`, `model_hash`) + **`model_identity_from_model()`** in **`modules/runtime_context.py`**; **`p.runtime_context.model_identity`** populated in **`process_images_inner`** from the same authoritative **`shared.sd_model`** object as before; **`p.sd_model_name`** / **`p.sd_model_hash`** assigned from **`model_identity`**. Tests: **`test/quality/test_runtime_mock.py`**. Remaining **`shared.sd_model`** reads in **`processing.py`** **deferred to M35** (see **`docs/milestones/M34/M34_plan.md`**). **`docs/architecture/serena_allowed_legacy_surfaces.md`** updated with M34 note.
...
**M33 ? Release-ready 5/5 close** **complete** — **PR #88** → merge **`ebb44177`**; **`docs/milestones/M33/`**; **Phase VII** **closed** at **M33**; **doc-only**; tag **`v0.0.33-m33`** @ **`ebb44177`**; provenance **`M33_run1.md`** §9. **Phase VIII:** **M34** **in progress** (`m34-runtime-context-model-identity`); **M35–M37** **planned** — **`docs/serenav1audit.md`**.
