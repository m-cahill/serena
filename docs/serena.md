...
| M22 | txt2img/img2img tab modularization | In progress | m22-tab-modularization | ? | ? | PR pending | ? | ? |
...
**M21:** Introduced `modules/ui_tab_registry.py` (`TabSpec`, `core_tab_specs`, merge of `ui_tabs_callback()` rows, Settings/Extensions append) and refactored `create_ui()` top-level `interfaces` assembly only. Preserved callback side-effect order, `shared.tab_names` pre-sort semantics, and `sorted_interfaces` / `hidden_tabs`. Quality **`test/quality/test_ui_tab_registry.py`**. PR **#40** @ **081de7e7**; tag **`v0.0.21-m21`**.

**M22:** *(in progress)* Extracted txt2img and img2img top-level `gr.Blocks` bodies to `modules/ui_txt2img_tab.py` and `modules/ui_img2img_tab.py` with `TabBuildResult` (`dummy_component`, `txt2img_preview_params`, `image_cfg_scale` for remaining `create_ui()` wiring). Registry public API unchanged. Import-light module entry points; lazy imports inside builders. Quality **`test/quality/test_ui_tab_modularization.py`**.
