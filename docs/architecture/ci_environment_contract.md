...
**Committed manifests are the source of truth.** For the Quality workflow, **CI environments are reproducible from `requirements-ci.txt`**, a **pinned OpenAI CLIP URL** installed with fixed `pip` flags (workflow step, same commit as the lockfile), and **`pip-audit`** run as a **blocking merge gate** (artifact + **job fails** on unresolved advisories per **pip-audit policy** below; **M28a+**). **Radon** runs on **`modules/`** for **cyclomatic complexity visibility** (artifact + optional warning for grade **D/E/F**; **non-blocking** through M27 per **complexity policy** below). **M29:** **`performance_snapshot.txt`** is generated after tests and uploaded as a **non-blocking** artifact — see **`docs/architecture/performance_baseline.md`**. **M41:** **`scripts/ci/check_performance_regression.py`** compares the snapshot to **`scripts/ci/performance_snapshot_baseline.txt`** and emits **workflow warnings** on **>20%** slowdown vs baseline (warn-first; does not fail the job). For JavaScript lint, **CI installs are reproducible from `package-lock.json` via `npm ci`.**
...
4. **`pip-audit`** — runs after verify; output is captured to **`pip_audit_report.txt`** and uploaded as a CI artifact. **M28a+:** non-zero exit **fails the Quality job** (blocking). **M41:** Nightly uses the **same governed `--ignore-vuln` IDs** as Quality and is **blocking** on unresolved advisories (install tree differs from Quality; see **pip-audit policy**). See **pip-audit policy** below.
...
| **Nightly (M41+)** | **Blocking** on unresolved advisories, with the **same documented `--ignore-vuln`** lines as Quality. Runs after the Nightly runtime install (see `run_nightly_tests.yaml`); artifact **`pip_audit_report.txt`** uploaded. Not a PR merge gate (scheduled / manual only). |
